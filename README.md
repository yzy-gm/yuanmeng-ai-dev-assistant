...
**作者：不见星光**。这是一个与官方“元梦开发助手”并存的 MIT 开源本机 VS Code 伴随工具，为《元梦之星》创作者提供 UI、场景元件、ID、编组、Lua 与 API 检查能力。它读取官方命令生成的 `CustomUIData*.lua`，也可在用户明确选择后只读解析本人地图的 `LayerData.dat`、`LayerData-Auto.dat` 或 `LayerData.pbin`，建立可供 VSCode、CLI 和本机 AI 查询的结构化索引；不会修改、替换或重新分发官方扩展。

本项目是不见星光开发的第三方工具，并非腾讯或《元梦之星》官方产品，与腾讯不存在隶属或授权关系。

## 安装与卸载

从 [Visual Studio Marketplace](https://marketplace.visualstudio.com/items?itemName=bujianxingguang.yuanmeng-ai-dev-assistant) 安装，或从 [GitHub Releases](https://github.com/yzy-gm/yuanmeng-ai-dev-assistant/releases) 下载 VSIX，在 VS Code 中运行“扩展：从 VSIX 安装…”。需要 **VS Code >=1.134**；独立 CLI/MCP 需要系统 **Node.js >=20**。安装不会修改系统 `PATH`。

**0.6.0 全功能开源公告：** 原私有版的 UI、场景、Lua/API 审查、官方兼容审查、MCP、代码交付和 AI 玩法模拟现均以 MIT 许可证开放。完整说明见 [更新公告](RELEASE_NOTES.md)。

扩展 ID 保持 `bujianxingguang.yuanmeng-ai-dev-assistant`。市场安装且开启自动更新、版本兼容的用户可沿用更新渠道；关闭自动更新的用户请手动检查扩展更新。VSIX 安装用户请主动安装新版。新版成功启动后会显示一次本地公告，旧版尚未更新时无法显示这条新增提示。

工程中的 `.yuanmeng-inspector/` 数据继续按工程隔离并被 Git 忽略，开源不代表上传地图、缓存、日志或账号信息。
...
静态检查、单元测试、CLI 集成、Extension Host、VSIX Clean Profile、官方编辑器单人联动和多人实测分别记录。验收步骤见 `docs/editor-acceptance-checklist.md`，构建与发布流程见 `docs/public-release.md`。自动化通过不能替代官方编辑器画面或多人验收。
